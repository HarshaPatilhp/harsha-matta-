...
export default function Sidebar({ isOpen, setIsOpen, userRole }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar sidebar */}
      <div 
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-emerald-900 text-emerald-50 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        } flex flex-col shadow-2xl`}
      >
        <div className="flex items-center justify-between p-6 px-6 bg-emerald-950/50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-amber-500 rounded-full flex items-center justify-center text-emerald-950 shadow-lg">
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1.5 scrollbar-thin scrollbar-thumb-emerald-700">
          {links.map((link) => {
            ...
            return (
              ...
              >
                <Icon size={20} className={isActive ? 'text-amber-400' : 'text-emerald-300 group-hover:text-amber-300'} />
                <span>{link.name}</span>
                {link.highlight && (
        ...
        <div className="p-4 border-t border-emerald-800/50 mt-auto">
          <button className="flex items-center gap-3 px-4 py-3 w-full rounded-xl text-emerald-200 hover:bg-red-500/20 hover:text-red-400 transition-colors">
            <LogOut size={20} />
