...
export default function DashboardContent() {
    ...
    try {
      ...
      const qrScanner = new QrScanner(
        ...
        (result: any) => {
          ...
          console.log('QR Code detected by scanner:', qrData);
  ...
  const simulateScan = (qrCode: string) => {
    try {
      console.log('🔍 [QR LOOKUP] Starting QR scan processing:', qrCode);
      
      // 1. NORMALIZE SCANNED VALUE - Treat scanned QR value strictly as bookingId
      const scannedBookingId = String(qrCode).trim();
      console.log('🔍 [QR LOOKUP] Normalized scanned bookingId:', scannedBookingId);
      
      // 2. LOAD BOOKINGS SAFELY - Read localStorage key "temple_bookings"
      let bookingsArray: Booking[] = [];
      try {
        const bookingsJson = localStorage.getItem('temple_bookings');
        if (bookingsJson) {
          bookingsArray = JSON.parse(bookingsJson);
          console.log('🔍 [QR LOOKUP] Loaded bookings from localStorage:', bookingsArray.length, 'total');
        } else {
          console.log('🔍 [QR LOOKUP] No bookings found in localStorage');
        }
      } catch (error) {
        console.error('🔍 [QR LOOKUP] Failed to parse bookings from localStorage:', error);
        bookingsArray = [];
      }
      
      // 3. FIND BOOKING CORRECTLY - Match using String(booking.id).trim() === scannedBookingId
      let foundBooking = null;
      for (const booking of bookingsArray) {
        if (booking && booking.id) {
          const bookingIdStr = String(booking.id).trim();
          console.log('🔍 [QR LOOKUP] Comparing:', { 
            scannedBookingId, 
            bookingIdStr, 
            match: bookingIdStr === scannedBookingId 
          });
          
          if (bookingIdStr === scannedBookingId) {
      ...
      console.log('🔍 [QR LOOKUP] Final booking lookup result:', {
        scannedBookingId,
        totalBookings: bookingsArray.length,
        availableBookingIds: bookingsArray.map(b => b.id).filter(Boolean),
        foundBooking: foundBooking ? { id: foundBooking.id, devoteeName: foundBooking.devoteeName } : null
      });
      
      // 4. HANDLE BOOKING NOT FOUND
      if (!foundBooking) {
        console.log('❌ [QR LOOKUP] Booking not found for scanned ID:', scannedBookingId);
        setScanResult(`❌ Booking not found. Please rescan or contact admin.`);
        setTimeout(() => setScanResult(''), 5000);
        // Do NOT open modal, do NOT set scannedBooking state, allow scanner to continue
        return;
      }
      
      // 5. HANDLE BOOKING FOUND - Set scannedBooking state with FULL booking object
      console.log('✅ [QR LOOKUP] Booking found, setting modal state');
      setScannedBooking(foundBooking);
      setShowScanResultModal(true);
      
      // Stop QR scanner ONLY AFTER state is set successfully
      setTimeout(() => {
        if (qrScannerRef.current) {
          qrScannerRef.current.stop();
          setIsScanning(false);
          console.log('🔍 [QR LOOKUP] QR scanner stopped after successful booking lookup');
        }
      }, 100);
      
      // Continue with existing logic for updating booking status and scan history
      const bookingIndex = bookingsArray.findIndex(b => String(b.id).trim() === scannedBookingId);
      if (bookingIndex !== -1) {
        ...
        const lunchCount = foundBooking.lunchCount || (foundBooking.lunchRequired ? foundBooking.numberOfPeople : 0);
        const lunchInfo = lunchCount > 0 ? `${lunchCount} people registered for lunch` : 'No lunch registered';
        const gotra = foundBooking.gotra || '';
        ...
        const numberOfPeople = foundBooking.numberOfPeople || 1;

        // Add to scan history
        const scanEntry: ScanHistory = {
          id: Date.now(),
          devoteeName: foundBooking.devoteeName || foundBooking.fullName || 'Unknown Devotee',
          sevaName: foundBooking.sevaName,
          bookingId: foundBooking.id,
          scanTime: new Date().toLocaleString(),
          status: 'Verified'
        };
        
        console.log('🔍 [QR LOOKUP] Adding new scan entry:', scanEntry);
        setScanHistory(prev => {
          const newHistory = [scanEntry, ...prev];
          console.log('🔍 [QR LOOKUP] Updated scan history:', newHistory);
          
          // Save to localStorage for persistence
          try {
            localStorage.setItem('scanHistory', JSON.stringify(newHistory));
            console.log('🔍 [QR LOOKUP] Saved scan history to localStorage');
          } catch (error) {
            console.error('🔍 [QR LOOKUP] Error saving scan history:', error);
          }
        ...
        if (numberOfPeople > 1) {
          setTimeout(() => {
            showMemberEntryDialog(foundBooking.id, numberOfPeople);
          }, 100);
        }
      }
    } catch (error) {
      console.error('❌ [QR LOOKUP] Error processing QR code:', error);
  ...
  // QR Scan Result Modal - rendered at the end of component
  const renderScanResultModal = () => {
    if (!showScanResultModal || !scannedBooking) return null;
